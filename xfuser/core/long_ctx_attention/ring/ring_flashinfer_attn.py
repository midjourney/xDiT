...
from yunchang.ring.ring_flashinfer_attn import RingFlashInferAttnFunc
...
def xdit_ring_flashinfer_attn_forward(
    process_group,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
...
class xFuserRingFlashInferAttnFunc(RingFlashInferAttnFunc):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        if attn_layer is None:
            k = k.contiguous()
            v = v.contiguous()
        out, softmax_lse = xdit_ring_flashinfer_attn_forward(
            group,
            q,
            k,
            v,
...
def xdit_ring_flashinfer_attn_func(
    q,
    k,
    v,
    dropout_p=0.0,
    ...
    #    )
    #else:
    return xFuserRingFlashInferAttnFunc.apply(
        q,
        k,
        v,
        dropout_p,
