...
from yunchang.ring.ring_pytorch_attn import RingTorchAttnFunc
...
def xdit_ring_torch_attn_forward(
    process_group,
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
...
class xFuserRingTorchAttnFunc(RingTorchAttnFunc):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        if attn_layer is None:
            k = k.contiguous()
            v = v.contiguous()
        out, softmax_lse = xdit_ring_torch_attn_forward(
            group,
            q,
            k,
            v,
...
def xdit_ring_torch_attn_func(
    ...
    #    )
    #else:
    return xFuserRingTorchAttnFunc.apply(
        q,
        k,
        v,
        dropout_p,
