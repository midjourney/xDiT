...
from xfuser.core.long_ctx_attention.ring.ring_torch_attn import xdit_ring_torch_attn_func
...
class TestRingTorchAttn(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.batch_size = 1
        cls.num_heads = 4
    ...
    def test_xdit_ring_torch_attn_func(self):
        ...
        output = xdit_ring_torch_attn_func(
            q=local_q,
            k=local_k,
            v=local_v,
            dropout_p=0.0,
    ...
    def test_xdit_ring_torch_attn_func_joint_strategy_rear(self):
        ...
        output_rear = xdit_ring_torch_attn_func(
            q=local_q,
            k=local_k,
            v=local_v,
            dropout_p=0.0,
    ...
    def test_xdit_ring_torch_attn_func_joint_strategy_front(self):
        """Test ring flash attention with joint strategy"""
        q, k, v, local_q, local_k, local_v = self._create_test_tensors()
        joint_q, joint_k, joint_v, local_joint_q, local_joint_k, local_joint_v = self._create_test_tensors()
        ...
        # Test front joint strategy
        output_front = xdit_ring_torch_attn_func(
            q=local_q,
            k=local_k,
            v=local_v,
            dropout_p=0.0,
...
# torchrun --nproc_per_node=2 -m unittest tests/core/test_ring_torch_attn.py
